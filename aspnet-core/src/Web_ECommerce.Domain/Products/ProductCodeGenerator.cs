...
namespace Web_ECommerce.Products
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(Web_ECommerceConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(Web_ECommerceConsts.ProductIdentitySettingId, "Sản phẩm", Web_ECommerceConsts.ProductIdentitySettingPrefix, 1, 1));
