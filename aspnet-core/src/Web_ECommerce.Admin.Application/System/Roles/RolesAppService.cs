...
namespace Web_ECommerce.Admin.System.Roles
{
        ...
        CreateUpdateRoleDto>, IRolesAppService
    {
        ...
        public async override Task<RoleDto> CreateAsync(CreateUpdateRoleDto input)
        {
            ...
            if (isNameExisted)
            {
                throw new BusinessException(Web_ECommerceDomainErrorCodes.RoleNameAlreadyExists)
        ...
        public async Task<GetPermissionListResultDto> GetPermissionsAsync(string providerName, string providerKey)
        {
            ...
            var getGroups = await PermissionDefinitionManager.GetGroupsAsync();
            foreach (var group in getGroups.Where(x => x.Name.StartsWith("AbpIdentity") || x.Name.StartsWith("WebEcomAdmin")))
            {
